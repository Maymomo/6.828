...
static int sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	if ((perm & (~PTE_SYSCALL)) != 0 || (perm & (PTE_P | PTE_U)) == 0) {
		return -E_INVAL;
	}
			...
			void *dstva, int perm)
{
	...
	if ((size_t)srcva >= UTOP || ((size_t)srcva & 0xfff) != 0) {
		return -E_INVAL;
	}

	if ((size_t)dstva >= UTOP || ((size_t)dstva & 0xfff) != 0) {
		return -E_INVAL;
	}
	...
	if (perm & PTE_W && ((size_t)(*src_pte) & PTE_W) == 0) {
		return -E_INVAL;
	}

	ret = page_insert(dst_env->env_pgdir, src_page, dstva, perm);
...
static int sys_page_unmap(envid_t envid, void *va)
{
	...
	if ((size_t)va >= UTOP || ((size_t)va & 0xfff) != 0) {
		return -E_INVAL;
	}
			    ...
			    unsigned perm)
{
	...
	ret = envid2env(envid, &target_env, false);
	if (ret != 0) {
		return ret;
	}
	...
	if (srcva != NULL) {
		...
		if ((size_t)target_env->env_ipc_dstva >= UTOP) {
			ret = -E_INVAL;
			return ret;
		}
		ret = page_insert(target_env->env_pgdir, page,
	...
	if (target_env->env_status != ENV_RUNNING) {
